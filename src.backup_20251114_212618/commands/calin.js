...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('calin')
    .setDescription('Faire un câlin quelqu\'un')
  ...
  async autocomplete(interaction) {
    ...
    if (focusedOption.name === 'zone') {
      try {
        ...
        const actionConfig = eco?.actions?.config?.['calin'] || {};
        ...
        console.error(`[Autocomplete] Error for calin:`, error.message);
        return interaction.respond([]);
      }
    }
  },
  async execute(interaction) {
    if (global.handleEconomyAction) {
      return global.handleEconomyAction(interaction, 'calin');
    } else {
