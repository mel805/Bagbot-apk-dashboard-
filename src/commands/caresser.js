...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('caresser')
    .setDescription('Caresser quelqu\'un')
    ...
    .addStringOption(option =>
      option.setName('zone')
        .setDescription('Zone à caresser')
  ...
  async autocomplete(interaction) {
    ...
    if (focusedOption.name === 'zone') {
      try {
        ...
        const actionConfig = eco?.actions?.config?.['caress'] || {};
        ...
        console.error(`[Autocomplete] Error for caresser:`, error.message);
        return interaction.respond([]);
      }
    }
  },
  async execute(interaction) {
    if (global.handleEconomyAction) {
      return global.handleEconomyAction(interaction, 'caress');
    } else {
