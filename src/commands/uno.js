  ...
  for (let i = 0; i < botPlayer.hand.length; i++) {
    ...
    if (card.type === 'wild' || card.type === 'wild_draw4') {
      ...
      if (result.success) {
        ...
        // 🎴 Notifier la couleur choisie par le bot
        const botPlayer = game.players.find(p => p.id === 'UNO_BOT_AI');
        if (botPlayer) {
...
// ✨ Fonction helper pour notifier le joueur actuel
async function notifyCurrentPlayer(game, channel) {
  const currentPlayer = game.getCurrentPlayer();
  if (!currentPlayer || currentPlayer.id === 'UNO_BOT_AI') return;
  
  try {
    // Message visible par tous dans le canal (auto-suppression après 5 secondes)
    const notifMsg = await channel.send({ 
      content: `🎯 <@${currentPlayer.id}> **C'est votre tour !** Cliquez sur 🃏 Ma main pour jouer.`,
      allowedMentions: { users: [currentPlayer.id] }
    });
    
    // Supprimer le message après 5 secondes pour ne pas spam le canal
...
// ✨ Fonction helper pour notifier le choix de couleur
async function notifyColorChoice(game, channel, playerId, playerName, color) {
  ...
  try {
    ...
    // Message visible par tous (auto-suppression après 5 secondes)
    const notifMsg = await channel.send({ 
      content: `🎴 **${playerName}** a choisi ${emoji} **${colorName}** !`,
...
// ✨ Fonction helper pour supprimer tous les messages "Ma main"
async function closeAllHandMessages(game) {
  ...
  for (const [playerId, handMsg] of game.playerHandMessages) {
    try {
      ...
      console.log(`[UNO] Message "Ma main" supprimé pour ${playerId}`);
    } catch (err) {
      console.log(`[UNO] Erreur suppression message main:`, err.message);
    }
  }
  game.playerHandMessages.clear();
  console.log(`[UNO] Tous les messages "Ma main" fermés`);
}
  ...
  async execute(interaction) {
    ...
    collector.on("collect", async i => {
      ...
      if (action === "join") {
        ...
        // ✅ DEFER IMMEDIAT pour eviter timeout Discord
  ...
  gameCollector.on("collect", async i => {
    ...
    if (action === "hand") {
      ...
      handCollector.on("collect", async handInteraction => {
        ...
        if (handInteraction.customId === `uno_${game.gameId}_draw`) {
          ...
          try {
            ...
            console.error(`[UNO DRAW HAND] Defer échoué:`, err.message);
            return; // Interaction expirée, ignorer
          }
          ...
          if (currentPlayer.id !== handInteraction.user.id) {
            ...
            return handInteraction.editReply({ 
              content: `❌ **Pas votre tour !**\n\n🎯 C'est au tour de **${currentPlayer.username}**\n⏳ Veuillez patienter...` 
            }).catch(() => {});
          }
        ...
        if (handInteraction.customId === `uno_${game.gameId}_uno`) {
          ...
          try {
            ...
            console.error(`[UNO UNO HAND] Defer échoué:`, err.message);
            return; // Interaction expirée, ignorer
          }
          ...
          if (player.hand.length === 1) {
            ...
            // 🎯 Notifier le joueur actuel (pas besoin ici, c'est juste dire UNO)
          } else {
        ...
        // Vérifier que l'interaction n'a pas déjà expiré
        try {
          if (!handInteraction.deferred && !handInteraction.replied) {
          ...
          // Si le defer échoue (interaction expirée), ne pas continuer
          console.log(`[UNO] Interaction expirée, ignore l'action`);
          return;
        }
            ...
            colorCollector.on("collect", async colorInteraction => {
              if (colorInteraction.customId.startsWith(`uno_${game.gameId}_color_`)) {
                ...
                try {
                  ...
                  console.error(`[UNO COLOR] Defer échoué:`, err.message);
                  colorCollector.stop();
                  return;
                }
                ...
                // ⚡ RÉPONSE IMMÉDIATE à l'interaction pour éviter "réfléchit" bloqué
                if (result.winner) {
                ...
                // 🎴 Notifier la couleur choisie à tous les joueurs
      ...
      try {
        ...
        console.error(`[UNO DRAW GAME] Defer échoué:`, err.message);
        return; // Interaction expirée, ignorer
      }
      ...
      try {
        ...
        console.error(`[UNO UNO GAME] Defer échoué:`, err.message);
        return; // Interaction expirée, ignorer
      }
      ...
      if (player.hand.length === 1) {
        ...
        // 🎯 Notifier le joueur actuel (pas nécessaire ici, dire UNO ne change pas le tour)
      } else {
      ...
      try {
        ...
        console.error(`[UNO CHALLENGE] Defer échoué:`, err.message);
        return; // Interaction expirée, ignorer
      }
      ...
      try {
        ...
        console.error(`[UNO QUIT] Defer échoué:`, err.message);
        // Si le defer échoue, essayer une simple reply
        try {
          await i.reply({ content: "🛑 Impossible de quitter (interaction expirée)", ephemeral: true });
        } catch (e) {}
        return;
      }
      ...
      if (game.isSoloMode) {
        ...
        gameCollector.stop();
        game.activeCollectors.forEach(c => c.stop());
        await i.editReply({ content: "🛑 Partie Solo annulée." });
        await message.edit({ content: "🛑 Partie annulée.", embeds: [], components: [] });
        return;
      }
      ...
      await i.editReply({ content: "✅ Vous avez quitté." });
