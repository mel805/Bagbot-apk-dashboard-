...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option.setName('zone')
        .setDescription('Zone à embrasser')
  ...
  async autocomplete(interaction) {
    ...
    if (focusedOption.name === 'zone') {
      try {
        ...
        const actionConfig = eco?.actions?.config?.['kiss'] || {};
        ...
        console.error(`[Autocomplete] Error for embrasser (uses kiss):`, error.message);
        return interaction.respond([]);
      }
    }
  },
  async execute(interaction) {
    if (global.handleEconomyAction) {
      return global.handleEconomyAction(interaction, 'kiss');
    } else {
